...
def args():
    parser = argparse.ArgumentParser(
        description="VM keystrokes using the vSphere API")
    ...
    parser.add_argument('--port', type=int, default=443,
                        help="alternative TCP port to communicate with vSphere API (default: 443)")
    parser.add_argument('--timeout', type=int, default=10,
                        help="timeout for VSphere API connection (default: 10s)")
    parser.add_argument('--debug', action='store_true',
                        help="Enable debug mode")
    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument(
        '--key', type=str, choices=[key[0] for key in HIDCode], help="key to passed to VM")
    group.add_argument('--string', help="string to passed to VM (Standard ASCII characters only)")
